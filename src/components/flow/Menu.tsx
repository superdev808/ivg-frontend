...
import { setSelectedFlow, setSelectedStart, setSelectedFlowData , setSelectedProcess} from '@/redux/features/flowSlice';
...
export default function FlowMenu() {
    ...
    const [avaliableFlows, setAvaliableFlows] = useState<any>([]);
    ...
    useEffect(() => {  
        const _avaliableFlows = flows?.filter((el: { node_type: Number }) => el.node_type === 0) 
        setAvaliableFlows(_avaliableFlows);
        setFilteredFlows(_avaliableFlows);
	...
	const handleCompleteMethod = (event: any) => {
		setTimeout(() => {
			...
			if (!event.query.trim().length) {
				_filteredArray = [...(avaliableFlows || [])];
			} else {
				_filteredArray = avaliableFlows?.filter((el: { text_1: string }) => {
	...
	const handleOnSelect= (event:any)=>{
        const _selectedFlowData:any  = flows?.filter(el => el.flow_id === event.id);
			...
			<Card className="flow-menu-container ">
                ...
                <div className='flow-menu-body'>
                            {(filteredFlows || []).map((item: { text_1: string; id: string }) => {
								return (
									<div
										className={'nav-link flex justify-content-between my-2 flow-menu-item' + (selectedFlow?.id === item.id ? ' active' : '')}
										key={'parent-' + item.id}
